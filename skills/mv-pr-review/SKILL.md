...
# mv-pr-review — 5-Lane Parallel PR Review
...
- *Blocker 0 + Major ≤ 3* 이면 `mv-verify-merge`로 진행 가능 표시.
...
- **자동 머지 금지**. 이 스킬은 *코멘트만*. 머지 결정은 사람 + `mv-verify-merge`.
