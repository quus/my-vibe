...
# mv-tdd-impl — Worktree-Based TDD Implementation (GREEN + REFACTOR)
...
## 2. Preconditions
- 해당 Story의 RED 테스트가 *존재하고 실패*하는 상태(보통 `mv-tdd-redgen` 직후).
...
## 6. Guardrails
- **테스트 수정 금지.** RED가 잘못이라 판단되면 *멈추고* 사람에게 보고 → `mv-tdd-redgen` 재실행.
...
## 8. Chains
- 선행: `mv-tdd-redgen` (RED 테스트 필요)
- 후행: `mv-pr-review` → `mv-verify-merge`

## 9. When NOT to use
- RED 테스트가 없거나 통과 중인 상태 — `mv-tdd-redgen` 먼저.
